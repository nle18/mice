"""Log probability version of mixture_model.py, where everything is computed in 
log-space
"""
...
def compute_prompt_probs_similar(
    ...
    # logsumexp to get the denominator
    denom = logsumexp(prompt_scores)

    # map it back to a dictionary
    prompt_logprobs = {
        prompt_id: prompt_scores[i] - denom for i, prompt_id in enumerate(prompt_ids)
    }
...
def compute_and_save_priors(
    ...
    if prior_distribution == "uniform":
        ...
        priors_filepath = os.path.join(
            example_dir,
            "%s_%s_log_priors.json" % (prior_distribution, similarScores_combine),
        )
    if os.path.exists(priors_filepath):
        ...
        if prior_distribution == "uniform":
            ...
            priors = compute_prompt_probs_similar(
                example_predictions, similarity_lst, similarScores_combine
            )
...
def mixtureModel(mention_probs_verbose, prompt_probs, predictions):
    ...
    for mention, mention_d in mention_probs_verbose.items():
        all_probs[mention] = logsumexp(
            [
                np.log(prob) + prompt_probs[prompt]
                for prompt, prob in mention_d["probs"].items()
            ]
        )
...
def main():
    ...
    for example_id, example in test_data.items():
        ...
        mention_probs_filepath = os.path.join(
            example_dir, "mixtureModel_%s_raw_mention_log_probs.json" % model_name
        )
        write_json(raw_mention_probs, mention_probs_filepath)
        weighted_mention_probs_filepath = os.path.join(
            example_dir,
            "mixtureModel_%s_weighted_mention_log_probs.json" % model_name,
        )
    ...
    predictions_filepath = os.path.join(
        exp_dir, "mixtureModel_%s_log_predictions.json" % model_name
    )
